...
import {SONG_DETAILS} from '../constants/urls';
...
function SongDetails() {
  ...
  useEffect(()=>{
    const fetchdata=async ()=>{
      setisloading(true)
       await axios.get(SONG_DETAILS+id).then((res)=>{
  ...
  (
    <div className="content">
            ...
            <div className="flex-none lg:flex">
              ...
              <div className="flex-auto ml-3 justify-start py-2">
                ...
                <div className="w-full flex-none text-xs text-green-500 dark:text-green-200 font-medium ">
                  ...
                  <h2 className="flex-auto text-black dark:text-white text-2xl md:text-4xl font-medium">{songdetails.song}</h2>
                </div>
                <div className="flex-auto  justify-start py-2" >
                    <p className="text-gray-500 font-semibold dark:text-gray-500">{songdetails.album} · {songdetails.music}</p>
                    <p className="text-gray-500 font-semibold dark:text-gray-500">{songdetails.singers}</p>
                    ...
                    <p className="text-gray-500 font-semibold dark:text-gray-500">{songdetails.primary_artists}</p>
                    <p className="text-gray-500 font-semibold dark:text-gray-500">{songdetails.copyright_text}</p>
                
                </div>
              </div>
              <div className="flex-auto ml-3 justify-start py-2">
                <div className="flex flex-wrap pt-0  md:pl-0 lg:pl-0 sm:pl-0 pmd:pt-14 lg:pt-14 sm:pt-14">
                   <PlayBtn link={songdetails.media_url}/>
               </div>
              </div>
            </div>
          </div>
      ...
      <div className="grid grid-cols-3 ">
        <h2 className="text-black  dark:text-white py-4 font-bold  text-3xl col-span-2">lyrics
        </h2>   
      </div>
      <div>
