...
import PercentageCalc from "../../helpers/PercentageCalc";
import TimeCalc from "../../helpers/Timecalc";
import { useAudio,useTitle } from "react-use";

function BottomNav(props) {
  const { image, media_url, song, album } = props.song;
        ...
        <div className="bg-rows-2">
          <div className="bg-gray-900 w-full h-2">
            <div
              ...
              style={{
                width: `${PercentageCalc(state.time, state.duration)}%`,
              }}
            ></div>
          </div>
          <div className=" bg-gray-200  dark:bg-gray-900 grid grid-cols-3">
            ...
            <div className=" text-black dark:bg-gray-900 dark:text-white  py-4 px-1 sm:px-3 lg:px-1 xl:px-3 grid grid-cols-5 sm:grid-cols-7 lg:grid-cols-5 xl:grid-cols-7 items-center">
              <button
                type="button"
                className="hidden sm:block lg:hidden xl:block mx-auto focus:outline-none"
              >
                <MdSkipPrevious className="text-4xl" />
              </button>
              <button
                type="button"
                className="hidden  sm:block focus:outline-none mx-auto"
              ...
              {isplay ? (
                <button
                  type="button"
                  onClick={handelPlay}
                  className="mx-auto h-50 w-50 shadow-xl focus:outline-none rounded-full"
                >
              ...
              <button
                type="button"
                className="hidden sm:block mx-auto focus:outline-none"
                onClick={() => controls.seek(state.time + 30)}
              >
                <svg width="34" height="39" fill="none">
                  <path
              ...
              <p className="hidden  sm:block ">
                {state.time && !isNaN(state.time) && TimeCalc(state.time)}/
                {state.duration &&
                  !isNaN(state.duration) &&
                  TimeCalc(state.duration)}
              </p>
