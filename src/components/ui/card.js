...
function Card(prop) {
  return (
    <div >
      <Link to={`/${prop.genre}/${prop.id}/${prop.title}`}>
      <div  className="relative bg-gray-900 shadow-2xl  rounded-lg group h-20 w-20 sm:h-48 sm:w-48 flex justify-center items-center">
        ...
        >
            <div>
            {/* <p className="  w-12 h-6 p-1 bg-green-500 font-bold text-white text-xs sm:text-xs  rounded-r-lg capitalize">{prop.lang}</p> */}
        </div>
        </div>
      </div>
      </Link>
